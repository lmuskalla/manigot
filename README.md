...
| `mg done` | archive a finished job — merges it into the base branch and removes its worktree; a conflicted merge offers to hand the job to @git-solver (via `mg host`) to resolve and finish the cleanup, and rolls back on decline; needs `docs/` |
