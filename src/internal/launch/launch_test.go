...
func TestAgentAndQuickShareOneTrackedPane(t *testing.T) {
	...
	// The inner command is the running binary itself (os.Executable) — in a
	// test that's the test binary, which never actually runs because the tmux
	// stub intercepts split-window before the pane command executes.
	root := t.TempDir()
...
// stubJdiExe points the launch package's executable lookup at stub and
// restores it after the test.
func stubJdiExe(t *testing.T, stub string) {
	t.Helper()
	old := JdiExe
	JdiExe = func() (string, error) { return stub, nil }
	t.Cleanup(func() { JdiExe = old })
}

func TestJdiExeLookupFailure(t *testing.T) {
	old := JdiExe
	defer func() { JdiExe = old }()
	JdiExe = func() (string, error) { return "", errors.New("boom") }

	err := Jdi("ab0001_x", t.TempDir(), "")
	if err == nil || !strings.Contains(err.Error(), "boom") {
		t.Errorf("expected the executable lookup error, got: %v", err)
	}
}

// The jdi subcommand must be started detached (no terminal emulator involved
// at all, unlike Agent/Quick), by absolute path, in projectRoot — the same
// invocation contract the one binary's `mg jdi` subcommand satisfies.
...
// forwarded to the mg jdi subcommand as its own --profile flag unchanged, not
// overridden by the claude-pro default TestJdiStartsResolvedCommandDetached
// exercises.
