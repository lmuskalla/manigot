...
func runJDI(args []string, stdout, stderr io.Writer) int {
	...
	profileArg := fs.String("profile", "", "subscription profile to run agents under: claude-pro, zai, opencode-go, opencode-zen, or opencode-zen-free (default claude-pro)")
	fs.Usage = func() {
	...
	if _, ok := config.ProfileByID(profile); !ok {
		fmt.Fprintf(stderr, "mg jdi: --profile must be one of: claude-pro, zai, opencode-go, opencode-zen, opencode-zen-free (got %q)\n", profile)
		return 2
	}
