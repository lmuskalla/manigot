...
// stubJdiExe points launch.JdiExe at stub and restores it after the test —
// the internal/ui equivalent of the seam internal/launch's own tests use,
// since os.Executable() (the real default) would otherwise start a second
// copy of this very test binary.
...
// failJdiExe points launch.JdiExe at a resolution failure and restores it
// after the test.
func failJdiExe(t *testing.T) {
	t.Helper()
	old := launch.JdiExe
	launch.JdiExe = func() (string, error) { return "", errors.New("mg-jdi: not found") }
	t.Cleanup(func() { launch.JdiExe = old })
}
...
func TestJdiKeyLaunchesDetachedAndSeedsBellDedup(t *testing.T) {
	...
	// Stub mg-jdi so no real container/process is spawned.
...
func TestJdiKeyBlocksSecondLaunchWhenSidecarSaysRunning(t *testing.T) {
	...
	t.Setenv("MANIGOT_HOME", "")
	stubJdiExe(t, markerStub(t, marker))
